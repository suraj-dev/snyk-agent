...
export async function gitIsClean(
  ...
  if (res.code !== 0)
    throw new Error(`git status failed: ${res.stderr.trim()}`);
...
export async function gitCurrentBranch(cwd: string): Promise<string> {
  ...
  if (res.code !== 0)
    throw new Error(`git rev-parse failed: ${res.stderr.trim()}`);
...
export async function gitCreateBranch(
  ...
  if (res.code !== 0)
    throw new Error(`git checkout -b ${branch} failed: ${res.stderr.trim()}`);
}
...
export async function gitCheckoutFiles(
  ...
  if (res.code !== 0)
    throw new Error(`git checkout -- failed: ${res.stderr.trim()}`);
}
...
export async function gitPush(cwd: string, branch: string): Promise<void> {
  ...
  if (res.code !== 0) {
    throw new Error(
      `git push failed: ${res.stderr.trim() || res.stdout.trim()}`,
    );
  }
}
...
export async function gitCheckoutBranch(
  ...
  const res = await runCommand("git", ["checkout", branch], cwd);
  if (res.code !== 0)
    throw new Error(`git checkout ${branch} failed: ${res.stderr.trim()}`);
}
